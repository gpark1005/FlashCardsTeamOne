...
func ConfigureRouter(handler CardHandler) *mux.Router {
	r := mux.NewRouter()
	...
	r.HandleFunc("/flashcards/category/{category}", handler.GetByCategoryHandler).Methods("GET")
	r.HandleFunc("/flashcards/id/{id}", handler.GetByIdHandler).Methods("GET")
